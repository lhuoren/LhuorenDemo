...
//        LogUtils.e("calDev","calGCJ02toWGS84:"+"latitude:"+latitude+",longitude："+longitude);
        Point dev = calDev(latitude, longitude);
        double retLat = latitude - dev.getLatitude();
        double retLon = longitude - dev.getLongitude();
    ...
    static Point calDevPoint = null;

    private static Point calDev(double latitude, double longitude) {
        if (isOutOfChina(latitude, longitude)) {
            calDevPoint.setLatitude(latitude);
            calDevPoint.setLongitude(longitude);
            return calDevPoint;
        }
...
//        return new Point(dLat, dLon);
        if (calDevPoint == null) {
            calDevPoint = new Point(dLat, dLon);
        }
        calDevPoint.setLatitude(dLat);
        calDevPoint.setLongitude(dLon);
//        LogUtils.e("calDev","latitude:"+calDevPoint.latitude+",longitude："+calDevPoint.longitude);
        return calDevPoint;
    }
    ...
    public static double[] calGCJ02toBD09(double latitude, double longitude) {
        ...
        double retLat = z * sin(theta) + 0.006;
        double retLon = z * cos(theta) + 0.0065;
        return new double[]{retLat, retLon};
    }
    ...
    public static double[] calBD09toGCJ02(double latitude, double longitude) {
        ...
        double z = sqrt(x * x + y * y) - 0.00002 * sin(y * PI);
        double theta = atan2(y, x) - 0.000003 * cos(x * PI);
        double retLat = z * sin(theta);
        double retLon = z * cos(theta);
        return new double[]{retLat, retLon};
    }
...
//        }
    ...
    public static double[] calBDtoGD(double bd_lat, double bd_lon) {
