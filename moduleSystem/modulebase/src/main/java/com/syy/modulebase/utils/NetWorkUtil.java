...
public class NetWorkUtil {
    ...
    private static long lastHasNetTime = 0L;

    private static final String LAST_HAS_NET_TIME_STAMP = "LAST_HAS_NET_TIME_STAMP";
    ...
    public static boolean isAvailable(Context context) {
        ...
        if (available) {
            lastHasNetTime = TimeAsyncManager.getCurrentTimeMillis();
        }
        return available;
    }
    ...
    public static void saveLastConnectTime() {
        if (lastHasNetTime > 0L) {
            SPUtils.put(LAST_HAS_NET_TIME_STAMP, lastHasNetTime);
        }
    }

    public static long lastConnectTime() {
        if (lastHasNetTime > 0L) {
            return lastHasNetTime;
        }
        return SPUtils.getLong(LAST_HAS_NET_TIME_STAMP, lastHasNetTime);
    }
}