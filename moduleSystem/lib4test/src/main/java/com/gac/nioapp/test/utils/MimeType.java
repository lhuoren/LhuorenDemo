...
//        return EnumSet.of(JPEG, PNG, GIF, BMP, WEBP);
    ...
    public static Set<MimeType> ofVideo() {
        return EnumSet.of(MPEG, MPEG2, MP4, QUICKTIME, THREEGPP, THREEGPP2, MKV, WEBM, TS, AVI);
    }

    public static Set<MimeType> ofVideoUpload() {
        return EnumSet.of(MP4, QUICKTIME, THREEGPP);
    }

    public static boolean supportUploadVideoFormat(String mimeTypeName) {
        if (mimeTypeName == null) return false;

        Set<MimeType> mimeTypes = ofVideoUpload();
        for (MimeType mimeType : mimeTypes) {
            if (mimeType.getMimeTypeName().equals(mimeTypeName)) {
                return true;
            }
        }
        return false;
