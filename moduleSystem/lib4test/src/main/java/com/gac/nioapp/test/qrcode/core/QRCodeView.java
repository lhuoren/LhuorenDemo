package com.gac.nioapp.test.qrcode.core;
...
public abstract class QRCodeView extends RelativeLayout implements Camera.PreviewCallback {
    ...
    // 上次环境亮度记录的时间戳
    private long mLastAmbientBrightnessRecordTime = TimeAsyncManager.getCurrentTimeMillis();
    // 上次环境亮度记录的索引
     ...
     * =
     */
    public void setTipBackgroundRadius(int radius) {
        if (mScanBoxView != null) {
            mScanBoxView.setTipBackgroundRadius(radius);
        }
    }
    /**
     * 显示扫描框
    ...
    public void onPreviewFrame(final byte[] data, final Camera camera) {
        if (QRCodeUtil.isDebug()) {
            QRCodeUtil.d("两次 onPreviewFrame 时间间隔：" + (TimeAsyncManager.getCurrentTimeMillis() - mLastPreviewFrameTime));
            mLastPreviewFrameTime = TimeAsyncManager.getCurrentTimeMillis();
        }
    ...
    private void handleAmbientBrightness(byte[] data, Camera camera) {
        ...
        long currentTime = TimeAsyncManager.getCurrentTimeMillis();
    ...
    private boolean handleAutoZoom(PointF[] locationPoints, final String result) {
        ...
        if (TimeAsyncManager.getCurrentTimeMillis() - mLastAutoZoomTime < 1200) {
            return true;
        }
    ...
    private void startAutoZoom(int oldZoom, int newZoom, final String result) {
        ...
        mLastAutoZoomTime = TimeAsyncManager.getCurrentTimeMillis();
    }
