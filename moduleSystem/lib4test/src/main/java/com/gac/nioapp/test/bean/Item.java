...
public class Item implements Parcelable {
    ...
    public Item(long id, String mimeType, Uri uri) {
        this.id = id;
    ...
    public static Item valueOf(Cursor cursor) {
        @SuppressLint("Range") Item item = new Item(cursor.getLong(cursor.getColumnIndex(MediaStore.Files.FileColumns._ID)),
    ...
    public static int indexOfList(Uri uri, List<Uri> list) {
        int index = -1;

        for (int i = 0; i < list.size(); i++) {
            Uri uriObj = list.get(i);
            if (uriObj.compareTo(uri) == 0) {
                index = i;
            }
        }
