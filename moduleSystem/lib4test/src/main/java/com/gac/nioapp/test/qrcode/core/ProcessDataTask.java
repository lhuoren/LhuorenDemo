    ...
    protected ScanResult doInBackground(Void... params) {
        ...
        if (mPicturePath != null) {
            ...
            if (QRCodeUtil.isDebug()) {
                QRCodeUtil.d("两次任务执行的时间间隔：" + (TimeAsyncManager.getCurrentTimeMillis() - sLastStartTime));
                sLastStartTime = TimeAsyncManager.getCurrentTimeMillis();
            }
            long startTime = TimeAsyncManager.getCurrentTimeMillis();

            ScanResult scanResult = processData(qrCodeView);

            if (QRCodeUtil.isDebug()) {
                long time = TimeAsyncManager.getCurrentTimeMillis() - startTime;
