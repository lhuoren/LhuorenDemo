...
public class MediaGrid extends SquareFrameLayout implements View.OnClickListener {
    ...
    private void init(Context context) {
        ...
        mViewHint =  findViewById(R.id.view_hint);
    ...
    public void bindMedia(Item item,boolean isThumbVisible) {
        this.isThumbVisible = isThumbVisible;
        mMedia = item;
        setGifTag();
        initCheckView();
        setImage(isThumbVisible);
        setVideoDuration();
    }
    ...
    public void setCheckEnabled(boolean enabled) {
        ...
        mViewHint.setAlpha(enabled ? 0f : 0.5f);
    }
    ...
    private void setImage(boolean isThumbVisible) {
        if (mMedia.isGif()) {
            ...
            SelectionSpec.getInstance().imageEngine.loadThumbnail(getContext(), mPreBindInfo.mResize,
                    mPreBindInfo.mPlaceholder, mThumbnail, isThumbVisible ? mMedia.getContentUri() : null);
        }
    }
