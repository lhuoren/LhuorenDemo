...
public class RegexUtils {
    ...
    public static final Pattern URL_PATTERN2 = Pattern.compile(
            "^(((ht|f)tp(s?))\\://)?(www.|[a-zA-Z].)[a-zA-Z0-9\\-\\.]+\\.(com|edu|gov|mil|net|org|biz|info|name|museum|us|ca|uk)(\\:[0-9]+)*(/($|[a-zA-Z0-9\\.\\,\\;\\?\\'\\\\\\+&%\\$#\\=~_\\-]+))*$");


    //stov
    public static final Pattern URL_PATTERN3 = Pattern.compile(
            "(https?|ftp|file)://[-A-Za-z0-9+&@#/%?=~_|!:,.;]+[-A-Za-z0-9+&@#/%=~_|]");


    //精确
    //如需匹配无http开头的网址，可能会出现Android WebView无法打开网址的情况，可以采取自动添加http://的形式，例如：
    //Matcher matcher = pattern.matcher(mUrl);
    //mUrl= matcher.find() && matcher.start() == 0 ? mUrl : "http://"+mUrl;
    public static final Pattern URL_PATTERN4 = Pattern.compile(
            "([hH][tT]{2}[pP]://|[hH][tT]{2}[pP][sS]://|[wW]{3}.|[wW][aA][pP].|[fF][tT][pP].|[fF][iI][lL][eE].|[mM]\\.)[-A-Za-z0-9+&@#/%?=~_|!:,.;]+[-A-Za-z0-9+&@#/%=~_|]+[-A-Za-z0-9+&@#/%=~_|]");

    //ios
    public static final Pattern URL_PATTERN5 = Pattern.compile(
            "(((https|http|ftp|rtsp|mms)?):\\/\\/[0-9a-zA-Z\\_\\-]+(\\.[0-9a-zA-Z\\_\\-]+)+([0-9a-zA-Z\\_\\-.,@?^=%&:\\/~+#]*[0-9a-zA-Z\\_\\-@?^=%&\\/~+#])?)|([0-9a-zA-Z\\_\\-]+(\\.[0-9a-zA-Z\\_\\-]+)+([0-9a-zA-Z\\_\\-.,@?^=%&:\\/~+#]*[0-9a-zA-Z\\_\\-@?^=%&\\/~+#])?)");
