...
/**
 * Created by lhuoren on 2015/10/18.
 */
class CubeTransitionIndicator : BaseIndicatorController() {
    ...
    override fun createAnimation(): List<Animator> {
        ...
        for (i in 0..1) {
            translateX[i] = startX
            var translationXAnim =
                ValueAnimator.ofFloat(startX, width - startX, width - startX, startX, startX)
            if (i == 1) {
        ...
        scaleAnim.start()
        val rotateAnim = ValueAnimator.ofFloat(0f, 180f, 360f, 1.5f * 360, (2 * 360).toFloat())
