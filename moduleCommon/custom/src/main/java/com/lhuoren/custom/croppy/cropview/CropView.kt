...
import com.lhuoren.custom.croppy.cropview.AspectMode.*
import com.lhuoren.custom.croppy.model.Corner.*
import com.lhuoren.custom.croppy.model.Corner.NONE
import com.lhuoren.custom.croppy.model.DraggingState.DraggingCorner
import com.lhuoren.custom.croppy.model.DraggingState.DraggingEdge
...
import com.lhuoren.custom.croppy.model.AnimatableRectF
import com.lhuoren.custom.croppy.AspectRatio
import com.lhuoren.custom.croppy.AspectRatio.*
import com.lhuoren.custom.croppy.model.Corner
import com.lhuoren.custom.croppy.CroppedBitmapData
import com.lhuoren.custom.croppy.CroppyTheme
import com.lhuoren.custom.croppy.model.DraggingState
import com.lhuoren.custom.croppy.model.Edge
import com.lhuoren.custom.croppy.extensions.animateScaleToPoint
import com.lhuoren.custom.croppy.extensions.animateTo
import com.lhuoren.custom.croppy.extensions.animateToMatrix
import com.lhuoren.custom.croppy.extensions.clone
import com.lhuoren.custom.croppy.extensions.getHypotenus
import com.lhuoren.custom.croppy.extensions.getTranslateY
...
class CropView @JvmOverloads constructor(
    ...
    private var observeCropRectOnOriginalBitmapChanged: ((RectF) -> Unit)? = null
     ...
     */
    private val touchThreshold = resources.getDimensionPixelSize(R.dimen.dimen_16).toFloat()
    ...
    private val emptyPaint = Paint().apply {
        isAntiAlias = true
        color = ContextCompat.getColor(context, R.color.color_E5FFFFFF)
    }
    ...
    private val marginInPixelSize =
        resources.getDimensionPixelSize(R.dimen.dimen_24).toFloat()
     ...
     */
    private val gridLineWidthInPixel = resources.getDimension(R.dimen.dimen_1)
     ...
     */
    private val cornerToggleWidthInPixel = resources.getDimension(R.dimen.dimen_6)

    /**
     * Corner toggle line length
     */
    private val cornerToggleLengthInPixel = resources.getDimension(R.dimen.dimen_32)

    private val minRectLength = resources.getDimension(R.dimen.dimen_55)
     ...
     * Mask color
     */
    private val maskBackgroundColor = ContextCompat.getColor(context, R.color.translucent_black_80)
    ...
    private val bitmapGestureListener = object : BitmapGestureHandler.BitmapGestureListener {
        override fun onDoubleTap(motionEvent: MotionEvent) {
            ...
            Log.i(
                "bitmapGestureListener",
                "onDoubleTap：${(isBitmapScaleExceedMaxLimit(DOUBLE_TAP_SCALE_FACTOR))}"
            )
...
//                val translateY = (viewHeight - bitmapRect.height() * scale) / 2f + marginInPixelSize
        ...
        override fun onSingleTap(motionEvent: MotionEvent) {
            singleClick?.singleClickTag()
        }

        override fun onScale(scaleFactor: Float, focusX: Float, focusY: Float) {
            Log.i(
                "bitmapGestureListener",
                "onDoubleTap：${(isBitmapScaleExceedMaxLimit(scaleFactor))}"
            )
            /**
        ...
        override fun onScroll(distanceX: Float, distanceY: Float) {
            Log.i("bitmapGestureListener", "onScroll：distanceX：$distanceX, distanceY:$distanceY")
            bitmapMatrix.postTranslate(-distanceX, -distanceY)
            invalidate()
        }

        override fun onEnd() {
            Log.i("bitmapGestureListener", "onEnd：distanceX：, distanceY:")
            settleDraggedBitmap()
        }
    }
    ...
    private var singleClick: onSingleClickListener? = null

    fun setOnSingleClickListener(singleClickListener: onSingleClickListener){
        this.singleClick = singleClickListener
    }
    ...
    init {
        ...
        setBackgroundColor(ContextCompat.getColor(context, R.color.color_FF000000))
    }
...
//                }
                ...
                draggingState = DraggingState.DraggingBitmap
    ...
    override fun onDraw(canvas: Canvas?) {
        ...
        canvas?.save()
        val clipRect = canvas?.clipRect(cropRect, Region.Op.DIFFERENCE)
        canvas?.drawColor(maskBackgroundColor)
        canvas?.restore()

//        drawGrid(canvas)
        fixedSizeDrawGrid(canvas)
...
//        drawCornerToggles(canvas)
    ...
    fun setBitmap(bitmap: Bitmap?) {
        ...
        Log.i("bitmapRect","bitmapRect：width："+this.bitmap?.width+",height："+this.bitmap?.height)
    ...
    private fun initialize() {
        ...
        val widthPixels = context.applicationContext.resources.displayMetrics.widthPixels
        val heightPixels = context.applicationContext.resources.displayMetrics.heightPixels
        Log.i(
            "initialize",
            "initialize,viewWidth：$viewWidth ,viewHeight:$viewHeight, widthPixels:$widthPixels, heightPixels:$heightPixels"
        )
        createMaskBitmap()
    ...
    private fun createMaskBitmap() {
        if (width > 0 && height > 0) else return
        Log.i(
            "createMaskBitmap",
            "createBitmap,measuredWidth：$measuredWidth ,measuredHeight:$measuredHeight"
        )
     ...
     * Draw crop rect as a grid.
     */
    private fun fixedSizeDrawGrid(canvas: Canvas?) {
        val width1 = context.applicationContext.resources.displayMetrics.widthPixels
        val height1 = context.applicationContext.resources.displayMetrics.heightPixels
        canvas?.drawRoundRect(
            (width * 0.025).toFloat(),
            (height * 0.32).toFloat(),
            (width * 0.972).toFloat(),
            (height.toFloat() * 0.638).toFloat(),
            mRoundCorner,
            mRoundCorner,
            paint
        )
...
//        )
    ...
    private var mPadding = 10f //间距
    private var mRoundCorner = 20f //圆角矩形的角度
    ...
    private fun aspectRatioChanged() {
        ...
        val translateX = (viewWidth - scale * bitmapRect.width()) / 2f + marginInPixelSize
        val translateY = (viewHeight - scale * bitmapRect.height()) / 2f + marginInPixelSize
        resetMatrix.postTranslate(translateX, translateY)
    ...
    private fun initializeBitmapMatrix() {
        Log.i(
            "initializeBitmapMatrix",
            "initializeBitmapMatrix,viewWidth：$viewWidth ,viewHeight:$viewHeight"
        )
...
//        val translateY = 0f
//        Log.i(
//            "initializeBitmapMatrix2",
//            "initializeBitmapMatrix2,translateX：$translateX ,translateY:$translateY"
//        )
//        bitmapMatrix.postTranslate(translateX, translateY)

        setInitialWidthAndHeightBitmap()
    }
...
//        bitmapMatrix.mapRect(cropRect, rect)
        ...
        val fixedRect = RectF(
            (width * 0.025).toFloat(),
            (height * 0.322).toFloat(),
            (width * 0.972).toFloat(),
            (height.toFloat() * 0.636).toFloat()
        )
//        val fixedRect = RectF(
//            (widthPixels * 0.025).toFloat(),
//            292.toFloat(),
//            (widthPixels * 0.972).toFloat(),
//            (heightPixels.toFloat() * 0.519).toFloat()
//        )

        cropRect.setTop(0f)
        cropRect.setBottom(0f)
        cropRect.setLeft(0f)
        cropRect.setRight(0f)
        bitmapMatrix.reset()
        bitmapMatrix.mapRect(cropRect, fixedRect)
        Log.i(
            "initializeCropRect",
            "initializeCropRect, widthPixels:$widthPixels, heightPixels:$heightPixels，cropRect：${cropRect},fixedRect:${fixedRect.top}，bitmapMatrix：${bitmapMatrix.getTranslateY()}"
        )
    }
                ...
                when (corner) {
                    TOP_RIGHT -> {
                        ...
                        val differenceHeight =
                            differenceWidth * selectedAspectRatio.heightRatio / selectedAspectRatio.widthRatio
            ...
            ASPECT -> {
                when (edge) {
                    ...
                    TOP -> {
                        ...
                        val differenceWidth =
                            differenceHeight * selectedAspectRatio.widthRatio / selectedAspectRatio.heightRatio
                        cropRect.setTop(cropRect.top + differenceHeight)
    ...
    private fun settleDraggedBitmap() {
        ...
        Log.i("settleDraggedBitmap","settleDraggedBitmap-widthScale:")
        /**
        ...
        Log.i("settleDraggedBitmap","widthScale:$widthScale,heightScale:$heightScale")
    ...
    private fun setInitialWidthAndHeightBitmap() {
        ...
        val widthPixels = context.applicationContext.resources.displayMetrics.widthPixels
        val heightPixels = context.applicationContext.resources.displayMetrics.heightPixels
        cropRect.setLeft((width * 0.025).toFloat())
        cropRect.setTop((height * 0.322).toFloat())
        cropRect.setRight((width * 0.972).toFloat())
        cropRect.setBottom((height.toFloat() * 0.636).toFloat())

//        cropRect.setLeft((widthPixels * 0.025).toFloat())
//        cropRect.setTop(292.toFloat())
//        cropRect.setRight((widthPixels * 0.972).toFloat())
//        cropRect.setBottom((heightPixels.toFloat() * 0.519).toFloat())

        val widthScale = cropRect.width() / draggedBitmapRect.width()
        val heightScale = cropRect.height() / draggedBitmapRect.height()
        var scale = 1.0f

        Log.i("settleDraggedBitmap",
            "cropRect:$cropRect,draggedBitmapRect:$draggedBitmapRect,widthScale:$widthScale,heightScale:$heightScale"
        )
        if (widthScale < 1.0f || heightScale < 1.0f) {
            scale = max(widthScale, heightScale)
        }
        ...
        if (scaledRect.bottom > cropRect.bottom) {
            translateY = (cropRect.bottom - scaledRect.bottom) / 2f + marginInPixelSize
        }

        Log.i("settleDraggedBitmap",
            "scaledRect:${scaledRect},cropRect:$cropRect,translateY:$translateY"
        )
        /**
         * New temp bitmap matrix
         */
    ...
    companion object {
        ...
        /**
         * Use this constant, when user single tap to scale
         */
        private const val SINGLE_TAP_SCALE_FACTOR = 1f
